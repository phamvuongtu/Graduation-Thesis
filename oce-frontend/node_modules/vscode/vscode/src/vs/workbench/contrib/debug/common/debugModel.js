    ...
    async fetchVariables(start, count, filter) {
        try {
            ...
            const vars = ( (response.body.variables.filter(v => !!v).map((v) => {
                ...
                return (
                     (new Variable(this.session, this.threadId, this, 0, '', undefined, ( localizeWithPath(_moduleId, 0, "Invalid variable attributes")), 0, 0, undefined, { kind: 'virtual' }, undefined, undefined, false))
                );
            })));
...
class Expression extends ExpressionContainer {
    static { this.DEFAULT_VALUE = ( localizeWithPath(_moduleId, 2, "not available")); }
    ...
    get stateLabel() {
        if (this.stoppedDetails) {
            return this.stoppedDetails.description ||
                (this.stoppedDetails.reason ? ( localizeWithPath(_moduleId, 3, "Paused on {0}", this.stoppedDetails.reason)) : ( localizeWithPath(_moduleId, 4, "Paused")));
        }
